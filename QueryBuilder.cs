...
namespace Lab3
{
    public class QueryBuilder : IDisposable
    {
        ...
        public void Update<T>(T obj) where T : Lab3.IClassModel
        {
        ...
        public void Delete<T>(T obj) where T : Lab3.IClassModel
        {
            var command = connection.CreateCommand();
        ...
        public bool DeleteAll<T>() where T : IClassModel // delete all records
        {
            try
            {
                var command = connection.CreateCommand();
        ...
        public bool Create<T>(T obj) where T : IClassModel
        {
            try
            {
                var command = connection.CreateCommand();
                ...
                command.CommandText = $"INSERT INTO {typeof(T).Name} ({columnNames}) VALUES ({parameterNames})";
